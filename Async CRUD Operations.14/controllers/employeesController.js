...
const getEmployee = async (req, res) => {
    ...
    if (!employee) {
        return res.sendStatus(204).json({ "message": `No employee matches ID ${req.params.id}. ` })
    }
    res.json(employee);
}
...
const updateEmployee = async (req, res) => {
    ...
    if (!employee) {
        return res.sendStatus(204).json({ "message": `No employee matches ID ${req.body.id}. ` })
    }
...
module.exports = {
    getEmployee, getAllEmployees, createNewEmployees, updateEmployee, deleteEmployee
}